...
// SVG icon components extracted from data array — inline JSX in object literals
// is not stable across renders; named components are properly memoized by React.
const LayersIcon = () => (
...
export default function Education() {
  return (
    <section id="education" className="py-20 sm:py-24 relative z-10 bg-[#0d1225]/80 backdrop-blur-sm border-y border-slate-800/50">
      <div className="max-w-6xl mx-auto px-4 sm:px-6">
        ...
        >
          ...
          <h2 className="text-3xl sm:text-4xl md:text-5xl font-extrabold text-slate-100">Education &amp; Certifications</h2>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <motion.div
            ...
            viewport={{ once: true }}
            transition={{ duration: 0.6 }}
            className="edu-degree-card lg:col-span-4 bg-gradient-to-br from-slate-900/80 to-slate-800/80 border border-slate-700/50 rounded-2xl p-8 shadow-[0_8px_32px_rgba(0,0,0,0.4)] backdrop-blur-md relative overflow-hidden group hover:border-cyan-400/50 transition-colors"
          >
          ...
          {/* Certifications — with cert-card class for reveal observer */}
          <div className="lg:col-span-8 grid grid-cols-1 sm:grid-cols-2 gap-4">
            {CERTS.map(({ name, issuer, year, iconClass, Icon }, i) => (
              <motion.div
                key={name}
                custom={i}
                ...
                className="cert-card flex items-center gap-4 bg-slate-900/40 border border-slate-700/40 rounded-xl p-5 hover:bg-slate-800/60 hover:border-slate-600/60 transition-colors group"
              >
                ...
                <div className="flex-1 min-w-0">
                  <h4 className="font-bold text-slate-200 text-sm line-clamp-2 leading-snug">{name}</h4>
                  <div className="text-xs text-slate-400 mt-1">{issuer}</div>
                </div>
                <div className="font-mono text-xs text-slate-500 flex-shrink-0">{year}</div>
              </motion.div>
            ))}
          </div>
